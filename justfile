...
# publish channel in CI — release.yml (installers + Homebrew), publish-crates.yml (crates.io
# via OIDC Trusted Publishing), publish-sdks.yml (the breadth SDK packages, npm today) AND,
# chained off release.yml's completion, publish-cli-npm.yml (the CLI's oura-toolkit npm
# launcher, OIDC, from the hosted release tarball).
...
release new_version:
    ...
    echo ""
    echo "Released ${tag}: CI is publishing installers + Homebrew (release.yml), crates.io (publish-crates.yml), the SDK packages (publish-sdks.yml) and the CLI npm launcher (publish-cli-npm.yml, after release.yml completes). Watch the Actions tab."
...
# .github/workflows/release.yml (installers + homebrew). Pushes with whatever git
...
release-tag new_version dry_run="false":
    ...
    # .github/workflows/release.yml (Homebrew + GitHub Release; publish-cli-npm.yml chains
    # off its completion for the npm launcher).
    echo "==> push ${tag}"
    if [ "$committed" = 1 ]; then git push --quiet origin HEAD:main; fi
    git push --quiet origin "${tag}"
    echo "Pushed ${tag}: CI is building installers + publishing Homebrew, crates.io, the SDK packages and the CLI npm launcher (watch the Actions tab)."
...
# init), which builds every platform and runs the homebrew publish job.
[group('release')]
dist-build:
    dist build
...
# Publish the CLI's npm launcher package (`oura-toolkit`, the NPX-FIRST channel) for a released
# tag — from the npm tarball dist BUILT and HOSTED on that tag's GitHub Release, so exactly the
# reviewed release artifact ships (this recipe builds nothing). CI runs this via
# .github/workflows/publish-cli-npm.yml after release.yml completes (dist 0.32's own npm publish
# job is token-only, so it's disabled in dist-workspace.toml and OIDC-published here instead).
# Auth comes from the environment (CI: Trusted Publishing / OIDC with automatic provenance;
# manual fallback: `npm login`). Same idempotency as sdk-publish-ts: skip if the version is
# already on the registry, and refuse a tarball whose version disagrees with the tag.
[group('release')]
publish-cli-npm tag:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    ver="${tag#v}"
    printf '%s' "$ver" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+([-.+][0-9A-Za-z.-]+)?$' \
      || { echo "publish-cli-npm: '${tag}' is not a release tag (expected vX.Y.Z)"; exit 1; }
    if [ "$(npm view "oura-toolkit@${ver}" version 2>/dev/null)" = "${ver}" ]; then
      echo "==> oura-toolkit@${ver} already on npm — skipping"
      exit 0
    fi
    # The artifact name is fixed by dist (asserted by dist-check); public repo, plain download.
    url="https://github.com/spxrogers/oura-toolkit/releases/download/${tag}/oura-toolkit-cli-npm-package.tar.gz"
    tmp="$(mktemp -d)"
    trap 'rm -rf "$tmp"' EXIT
    echo "==> fetching ${url}"
    curl --proto '=https' --tlsv1.2 -LsSf -o "$tmp/oura-toolkit-cli-npm-package.tar.gz" "$url"
    tarver="$(tar -xzOf "$tmp/oura-toolkit-cli-npm-package.tar.gz" package/package.json | jq -r .version)"
    [ "$tarver" = "$ver" ] || { echo "publish-cli-npm: release ${tag}'s npm tarball says version ${tarver} — refusing to publish a mismatched artifact"; exit 1; }
    echo "==> publishing oura-toolkit@${ver}"
    npm publish --access public "$tmp/oura-toolkit-cli-npm-package.tar.gz"
