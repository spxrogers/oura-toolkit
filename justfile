...
plugin-check:
    ...
    jq -e --arg v "{{version}}" '.mcpServers.oura.args | index("oura-toolkit@" + $v) != null' plugins/oura-toolkit/.mcp.json > /dev/null || { echo ".mcp.json must pin npx to oura-toolkit@{{version}} — sync it to the workspace Cargo.toml"; exit 1; }
