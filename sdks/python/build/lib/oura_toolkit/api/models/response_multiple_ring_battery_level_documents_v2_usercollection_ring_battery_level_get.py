...
RESPONSEMULTIPLERINGBATTERYLEVELDOCUMENTSV2USERCOLLECTIONRINGBATTERYLEVELGET_ANY_OF_SCHEMAS = ["TimeSeriesResponseDict", "TimeSeriesResponsePublicRingBatteryLevelRow"]

class ResponseMultipleRingBatteryLevelDocumentsV2UsercollectionRingBatteryLevelGet(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseMultipleRingBatteryLevelDocumentsV2UsercollectionRingBatteryLevelGet.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseMultipleRingBatteryLevelDocumentsV2UsercollectionRingBatteryLevelGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseMultipleRingBatteryLevelDocumentsV2UsercollectionRingBatteryLevelGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow. Details: " + ", ".join(error_messages))
        else:
            return instance
