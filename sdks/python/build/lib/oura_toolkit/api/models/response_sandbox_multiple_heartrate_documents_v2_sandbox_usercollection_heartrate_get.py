...
RESPONSESANDBOXMULTIPLEHEARTRATEDOCUMENTSV2SANDBOXUSERCOLLECTIONHEARTRATEGET_ANY_OF_SCHEMAS = ["TimeSeriesResponseDict", "TimeSeriesResponsePublicHeartRateRow"]

class ResponseSandboxMultipleHeartrateDocumentsV2SandboxUsercollectionHeartrateGet(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseSandboxMultipleHeartrateDocumentsV2SandboxUsercollectionHeartrateGet.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseSandboxMultipleHeartrateDocumentsV2SandboxUsercollectionHeartrateGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicHeartRateRow. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseSandboxMultipleHeartrateDocumentsV2SandboxUsercollectionHeartrateGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicHeartRateRow. Details: " + ", ".join(error_messages))
        else:
            return instance
