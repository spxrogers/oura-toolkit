...
RESPONSESANDBOXMULTIPLERINGBATTERYLEVELDOCUMENTSV2SANDBOXUSERCOLLECTIONRINGBATTERYLEVELGET_ANY_OF_SCHEMAS = ["TimeSeriesResponseDict", "TimeSeriesResponsePublicRingBatteryLevelRow"]

class ResponseSandboxMultipleRingBatteryLevelDocumentsV2SandboxUsercollectionRingBatteryLevelGet(BaseModel):
    """
    ResponseSandboxMultipleRingBatteryLevelDocumentsV2SandboxUsercollectionRingBatteryLevelGet
    """

    # data type: TimeSeriesResponsePublicRingBatteryLevelRow
    anyof_schema_1_validator: Optional[TimeSeriesResponsePublicRingBatteryLevelRow] = None
    # data type: TimeSeriesResponseDict
    anyof_schema_2_validator: Optional[TimeSeriesResponseDict] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "TimeSeriesResponseDict", "TimeSeriesResponsePublicRingBatteryLevelRow" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseSandboxMultipleRingBatteryLevelDocumentsV2SandboxUsercollectionRingBatteryLevelGet.model_construct()
        error_messages = []
        # validate data type: TimeSeriesResponsePublicRingBatteryLevelRow
        if not isinstance(v, TimeSeriesResponsePublicRingBatteryLevelRow):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TimeSeriesResponsePublicRingBatteryLevelRow`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ResponseSandboxMultipleRingBatteryLevelDocumentsV2SandboxUsercollectionRingBatteryLevelGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[TimeSeriesResponsePublicRingBatteryLevelRow] = None
        try:
            instance.actual_instance = TimeSeriesResponsePublicRingBatteryLevelRow.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResponseSandboxMultipleRingBatteryLevelDocumentsV2SandboxUsercollectionRingBatteryLevelGet with anyOf schemas: TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], TimeSeriesResponseDict, TimeSeriesResponsePublicRingBatteryLevelRow]]:
