...
class PublicRingBatteryLevelRow(BaseModel):
    """
    Object defining a ring battery level event.
    """ # noqa: E501
    timestamp: Optional[StrictStr]
    timestamp_unix: StrictInt = Field(description="Timestamp of the discrete sample as unix time in milliseconds.")
    charging: Optional[StrictBool] = None
    in_charger: Optional[StrictBool] = None
    level: StrictInt = Field(description="Ring battery level percentage. These values are within [0, 100].")
    __properties: ClassVar[List[str]] = ["timestamp", "timestamp_unix", "charging", "in_charger", "level"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PublicRingBatteryLevelRow from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if charging (nullable) is None
        # and model_fields_set contains the field
        if self.charging is None and "charging" in self.model_fields_set:
            _dict['charging'] = None

        # set to None if in_charger (nullable) is None
        # and model_fields_set contains the field
        if self.in_charger is None and "in_charger" in self.model_fields_set:
            _dict['in_charger'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PublicRingBatteryLevelRow from a dict"""
        if obj is None:
            return None
